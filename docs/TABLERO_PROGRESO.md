...
| 12 | Seguridad de usuarios con JWT y rol inicial | Completado | Pendiente | En progreso | Backend OK (`mvnw test`, `mvnw -DskipTests package`, `backend_smoke_local.ps1 -SkipBuild` con login JWT); frontend pendiente | Completada | Pendiente de registrar hash final | Pendiente | Backend protegido con Spring Security, JWT firmado, rol `OPERATOR`, usuario demo documentado y endpoint `/api/v1/users/me` listo para integracion cliente. |
