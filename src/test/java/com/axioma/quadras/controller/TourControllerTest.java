...
class TourControllerTest {
	...
	void shouldReturnPeriodSummary() throws Exception {
		...
		mockMvc.perform(get("/api/v1/tours/reports/summary")
				...
				.andExpect(jsonPath("$.providerBreakdown[0].groupKey").isNotEmpty())
				...
				.andExpect(jsonPath("$.serviceTypeBreakdown[0].groupKey").value("TOUR"))
				.andExpect(jsonPath("$.serviceTypeBreakdown[0].scheduledCount").value(2))
				.andExpect(jsonPath("$.serviceTypeBreakdown[1].groupKey").value("TRAVEL"))
				...
				.andExpect(jsonPath("$.paymentMethodBreakdown[1].groupKey").value("CARD"))
				.andExpect(jsonPath("$.paymentMethodBreakdown[1].grossAmount").value(400.00))
				.andExpect(jsonPath("$.paymentMethodBreakdown[2].groupKey").value("CASH"))
				.andExpect(jsonPath("$.paymentMethodBreakdown[2].grossAmount").value(0.00))
				.andExpect(jsonPath("$.paymentMethodBreakdown[3].groupKey").value("TRANSFER"))
	...
	void shouldReturnProviderSummaryDetails() throws Exception {
		...
		mockMvc.perform(get("/api/v1/tours/reports/summary/details")
						...
						.param("groupKey", String.valueOf(providerA.getId()))
						.param("dateFrom", "2026-04-01")
						.param("dateTo", "2026-04-30"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.groupBy").value("PROVIDER"))
				.andExpect(jsonPath("$.groupKey").value(String.valueOf(providerA.getId())))
				.andExpect(jsonPath("$.label").value("Agencia A"))
				.andExpect(jsonPath("$.summary.groupKey").value(String.valueOf(providerA.getId())))
	...
	void shouldReturnServiceTypeSummaryDetails() throws Exception {
		...
		mockMvc.perform(get("/api/v1/tours/reports/summary/details")
						...
						.param("groupKey", "TRAVEL")
						.param("dateFrom", "2026-04-01")
						.param("dateTo", "2026-04-30"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.groupKey").value("TRAVEL"))
